...
export default async function DashboardPage() {
  ...
  const [total, newLeads, hot, visits, activeMembers, followUps, packageGroups, recent] = await db.$transaction([
  ...
  const revenueByPackage = packageGroups.map((group) => ({
    ...group,
    netAmount: netRevenueAmount(group._sum?.amount || 0, group.packageId),
  }));
  const monthlyRevenue = revenueByPackage.reduce((sum, group) => sum + group.netAmount, 0);
  const metrics = [
  ...
  return <>
    ...
    <section className="mt-4 grid gap-4 lg:grid-cols-[1.2fr_1fr]">
      <div className="card overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 p-5"><div><p className="text-sm text-slate-500">Monthly revenue</p><h2 className="mt-1 text-3xl font-bold text-[#0b1f3a]">{formatRwf(monthlyRevenue)}</h2></div><div className="rounded-xl bg-[#fff5d8] p-3 text-[#9a7110]"><TrendingUp /></div></div>
        <div className="p-5"><h3 className="mb-4 text-sm font-bold text-slate-700">Revenue by package</h3>{revenueByPackage.length ? <div className="space-y-4">{revenueByPackage.map((group) => { const pct = monthlyRevenue ? Math.max(5, group.netAmount / monthlyRevenue * 100) : 0; return <div key={group.packageId || "none"}><div className="mb-1.5 flex justify-between gap-3 text-sm"><span className="truncate text-slate-600">{group.packageId ? packageMap.get(group.packageId) : "Unassigned"}</span><span className="font-semibold">{formatRwf(group.netAmount)}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#d4a72c]" style={{ width: `${pct}%` }} /></div></div>; })}</div> : <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No payments recorded this month yet.</p>}</div>
      </div>
