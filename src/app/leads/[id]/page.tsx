...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    <PageHeader eyebrow="Lead profile" title={name} description={`${lead.source} · Created ${formatDate(lead.createdAt)}`} action={<div className="flex flex-wrap gap-2"><a href={whatsappUrl(lead.phone)} target="_blank" rel="noreferrer" className="btn bg-[#168b5b] text-white"><MessageCircle size={17} />WhatsApp</a><Link href="/lead-assistant" className="btn btn-outline"><Bot size={16} />AI Lead Assistant</Link><Link href={`/leads/${lead.id}/edit`} className="btn btn-outline"><Edit3 size={16} />Edit</Link><PortalInviteButton leadId={lead.id} linked={Boolean(lead.portalLinkedAt)} /></div>} />
      ...
      <div className="space-y-4">
        <section className="card p-5"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 className="font-bold text-[#0b1f3a]">Lead overview</h2><QuickStatus leadId={lead.id} status={lead.status} /></div><div className="grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
          ...
          {(lead.nextAction || lead.requestedDate || lead.numberOfPeople || lead.equipmentRequest || lead.language) && <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {(lead.requestedDate || lead.requestedTime) && <div className="rounded-xl bg-slate-50 p-4 text-sm"><CalendarDays className="mb-2 text-[#9a7110]" size={18} /><p className="text-xs text-slate-500">Requested timing</p><p className="mt-1 font-semibold">{[lead.requestedDate, lead.requestedTime].filter(Boolean).join(" · ")}</p></div>}
            {lead.numberOfPeople && <div className="rounded-xl bg-slate-50 p-4 text-sm"><Users className="mb-2 text-[#9a7110]" size={18} /><p className="text-xs text-slate-500">People</p><p className="mt-1 font-semibold">{lead.numberOfPeople}</p></div>}
            {lead.equipmentRequest && <div className="rounded-xl bg-slate-50 p-4 text-sm"><Wrench className="mb-2 text-[#9a7110]" size={18} /><p className="text-xs text-slate-500">Equipment / setup</p><p className="mt-1 font-semibold">{lead.equipmentRequest}</p></div>}
            {lead.language && <div className="rounded-xl bg-slate-50 p-4 text-sm"><Languages className="mb-2 text-[#9a7110]" size={18} /><p className="text-xs text-slate-500">Language</p><p className="mt-1 font-semibold">{lead.language}</p></div>}
          </div>}
          {lead.importantNotes && <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6"><strong className="block text-blue-800">Important lead facts</strong><span className="whitespace-pre-line text-slate-700">{lead.importantNotes}</span></div>}
          {lead.notes && <div className="mt-4 rounded-xl border border-[#eadcae] bg-[#fffaf0] p-4 text-sm leading-6"><strong className="block text-[#76570b]">Staff notes</strong>{lead.notes}</div>}
        </section>
        <section className="card p-5"><h2 className="mb-4 font-bold text-[#0b1f3a]">Conversation timeline and notes</h2><NoteForm leadId={lead.id} /><div className="mt-5 space-y-3">{lead.interactions.map((item) => { const inbound = item.type === "whatsapp-in"; const outbound = item.type === "whatsapp-out"; const draft = item.type === "ai-draft"; return <div className={`flex gap-3 rounded-xl p-3 ${inbound ? "bg-blue-50" : outbound ? "bg-green-50" : draft ? "bg-amber-50" : ""}`} key={item.id}><div className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${inbound ? "bg-blue-500" : outbound ? "bg-green-500" : draft ? "bg-amber-400" : "bg-[#d4a72c]"}`} /><div className="min-w-0"><p className="whitespace-pre-line break-words text-sm leading-6 text-slate-700">{item.content}</p><p className="mt-1 text-xs text-slate-400">{inbound ? "Customer message" : outbound ? "Madar Hub sent" : draft ? "AI draft" : item.type} · {formatDate(item.createdAt, true)}</p></div></div>; })}{lead.interactions.length === 0 && <p className="text-sm text-slate-500">No interactions recorded yet.</p>}</div>{lead.rawWhatsappSnippet && <details className="mt-4"><summary className="cursor-pointer text-xs font-semibold text-slate-400 hover:text-slate-600">Initial WhatsApp transcript</summary><div className="mt-2 whitespace-pre-line rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-500">{lead.rawWhatsappSnippet}</div></details>}</section>
